...
def parse_args(argv=None):
    ...
    parser.add_argument(
        "--motion_file",
        type=str,
        default="/home/z/Desktop/PBHC/example/pretrained_horse_stance_pose/metrics/ckpt_50000/tmp.pkl",
        help="pass",
    )
    parser.add_argument(
        "--hang",
...
def main(argv = None) -> None:
    ...
    if args.stand_still:
        ...
        motion_data = {"stand": _make_stand_motion(T=args.frames, height=args.height)}
    ...
    mj_model.opt.timestep = min(mj_model.opt.timestep, 1/360)
    ...
    print("[INFO] Using PD control to maintain initial pose.")

    # 设置控制参数（可微调）
    kp = 200.0   # 比例增益（刚性）
    kd = 10.0    # 阻尼系数
    gravity_comp = True  # 是否补偿重力

    def apply_stabilizing_torque():
        """计算并施加力矩，使机器人维持初始站立姿态"""
        
        # 排除根节点的自由度：假设前7个是 root (3 平移 + 4 四元数)
        n_root = 7
        
        # 只控制剩余的关节部分
        pos_err = qpos_init[n_root:] - mj_data.qpos[n_root:]
        vel_err = -mj_data.qvel[n_root:]
        
        torque = kp * pos_err + kd * vel_err
        torque = np.clip(torque, -200, 200)

        print("[DEBUG]!!!!", mj_model.nu, len(torque))
        
        # 写入控制信号（ctrl 数量一般等于关节自由度）
        mj_data.ctrl[:] = torque
        print(f"Applied torque: {torque.shape} (len={len(torque)})")
    ...
    # 启动可视化线程
    thread = threading.Thread(target=run_viewer, args=(mj_model, mj_data))
    thread.start()

    # 主线程进行PD控制
    for step in range(1000):
        print("[DEBUG] Frame ID: ",curr_time,'\t | Times ',f"{time_step:4f}",end='\r\b')
        step_start = time.time()

        apply_stabilizing_torque()
